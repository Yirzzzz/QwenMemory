...
class AutoAdaptiveValueTracker:
    ...
    def update(self, rewards, cur_logprobs=None, response_masks=None):
        if cur_logprobs is not None and response_masks is not None:
            mean_logprob = ((cur_logprobs * response_masks).sum() / response_masks.sum().clamp_min(1)).item()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Qwen/MiniMind SPO")
    parser.add_argument("--save_dir", type=str, default="../out", help="模型保存目录")
    parser.add_argument('--save_weight', default='spo', type=str, help="保存权重前缀")
    ...
    parser.add_argument("--wandb_project", type=str, default="Qwen-SPO", help="wandb项目名")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"SPO-{args.model_source}-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
